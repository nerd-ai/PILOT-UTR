...
def _masked_motif_vocab_ce_loss(
  log_p_x0,
  log_p_x0_old,
  last_x,
  mask_idx,
  ...
  """
  Masked-token motif-vocab divergence between old/new models.
  Works directly in token vocab space [B, T, V] and averages only over masked tokens.
  """
  probs_new = log_p_x0.exp()
  probs_new = probs_new / probs_new.sum(dim=-1, keepdim=True).clamp_min(1e-12)
  probs_old = log_p_x0_old.exp()
  ...
  T = min(probs_new.shape[1], probs_old.shape[1], mask_tok.shape[1])
  if T <= 0:
    return torch.tensor(0.0, device=log_p_x0.device)
  mask = mask_tok[:, :T].to(probs_new.dtype)  # [B, T]
  probs_new = probs_new[:, :T, :]
  probs_old = probs_old[:, :T, :]

  mask_count = mask.sum()
  if mask_count.item() <= 0:
    return torch.tensor(0.0, device=log_p_x0.device)

  p_new = probs_new.clamp_min(1e-8)
  p_new = p_new / p_new.sum(dim=-1, keepdim=True).clamp_min(1e-8)
  p_old = probs_old.clamp_min(1e-8)
  p_old = p_old / p_old.sum(dim=-1, keepdim=True).clamp_min(1e-8)

  if int(vocab_topk) > 0 and int(vocab_topk) < int(p_new.shape[-1]):
    k = int(vocab_topk)
    if topk_source == "new":
      topk_ref = p_new
    elif topk_source == "union":
      topk_ref = None
    else:
      topk_ref = p_old

    if topk_ref is not None:
      topk_idx = torch.topk(topk_ref, k=k, dim=-1).indices
      topk_mask = torch.zeros_like(p_old, dtype=torch.bool)
      topk_mask.scatter_(-1, topk_idx, True)
    else:
      topk_idx_old = torch.topk(p_old, k=k, dim=-1).indices
      topk_idx_new = torch.topk(p_new, k=k, dim=-1).indices
      topk_mask = torch.zeros_like(p_old, dtype=torch.bool)
      topk_mask.scatter_(-1, topk_idx_old, True)
      topk_mask.scatter_(-1, topk_idx_new, True)

    p_old = p_old * topk_mask.to(p_old.dtype)
    p_new = p_new * topk_mask.to(p_new.dtype)
    p_old = p_old / p_old.sum(dim=-1, keepdim=True).clamp_min(1e-8)
    p_new = p_new / p_new.sum(dim=-1, keepdim=True).clamp_min(1e-8)
  ...
  if agg_method == "global":
    ...
    if not valid.any():
      return torch.tensor(0.0, device=log_p_x0.device)
    denom = per_sample_mask.clamp_min(1.0).unsqueeze(-1)  # [B, 1]
    p_new_s = (p_new * mask.unsqueeze(-1)).sum(dim=1) / denom  # [B, V]
    p_old_s = (p_old * mask.unsqueeze(-1)).sum(dim=1) / denom  # [B, V]
    p_new_s = p_new_s.clamp_min(1e-8)
    p_new_s = p_new_s / p_new_s.sum(dim=-1, keepdim=True).clamp_min(1e-8)
    p_old_s = p_old_s.clamp_min(1e-8)
    p_old_s = p_old_s / p_old_s.sum(dim=-1, keepdim=True).clamp_min(1e-8)
  ...
  # position-wise mode
  if divergence == "kl":
    loss_pos = (p_old * (p_old.log() - p_new.log())).sum(dim=-1)  # [B, T]
  else:
    loss_pos = -(p_old * p_new.log()).sum(dim=-1)  # [B, T]
  return (loss_pos * mask).sum() / mask_count.clamp_min(1.0)
...
def _masked_avg_vocab_probs(probs_row, last_x_row, mask_idx, token_valid_len=None):
  ...
  probs = probs / probs.sum(dim=-1, keepdim=True).clamp_min(1e-12)
  xt_ids = last_x_row[:T].argmax(dim=-1)
  masked = (xt_ids == int(mask_idx))
...
def fine_tune(new_model, new_model_y, new_model_y_eval, old_model, args, eps_ood=None, eps=1e-5):
  ...
  for epoch_num in range(args.num_epochs):
    ...
    ood_frac_list = []
    ood_score_mean_list = []
    drift_final_mask_new = []
    drift_final_mask_old = []
    drift_mask_curve_l1 = []
    drift_seq_hamming = []
    drift_seq_basecomp_l1 = []
    drift_seq_kmer_l1 = []
    tot_grad_norm = 0.0
      ...
      if args.enable_drift_monitor and (_step % max(1, args.drift_monitor_every) == 0):
        with torch.no_grad():
          ...
          if valid_mask_post is None:
            drift_final_mask_new.append(float(new_final_mask.mean().item()))
            drift_final_mask_old.append(float(old_final_mask.mean().item()))
          else:
            denom = valid_mask_post.sum().clamp_min(1.0)
            drift_final_mask_new.append(float(((new_final_mask * valid_mask_post).sum() / denom).item()))
            drift_final_mask_old.append(float(((old_final_mask * valid_mask_post).sum() / denom).item()))
          ...
          if Tm > 0:
            ...
            drift_mask_curve_l1.append(float(curve_l1))
          ...
          drift_seq_hamming.append(float(sum(hamming_vals) / max(1, len(hamming_vals))))
          drift_seq_basecomp_l1.append(float(sum(comp_vals) / max(1, len(comp_vals))))
          drift_seq_kmer_l1.append(float(sum(kmer_vals) / max(1, len(kmer_vals))))
      ...
      for random_t in range(args.total_num_steps):
        ...
        if (
          ...
          with torch.no_grad():
            ...
            if target_length is None:
              valid_len = last_x.shape[1]
            elif isinstance(target_length, torch.Tensor):
              valid_len = int(target_length[b_idx].item())
            else:
              valid_len = int(target_length[b_idx])

            mask_idx = int(getattr(new_model, "mask_index", -1))
            avg_new, mask_cnt = _masked_avg_vocab_probs(
              probs_t[b_idx], last_x[b_idx], mask_idx, token_valid_len=valid_len)
            avg_old, _ = _masked_avg_vocab_probs(
              probs_t_old[b_idx], last_x[b_idx], mask_idx, token_valid_len=valid_len)
            avg_new_str = _format_vocab_distribution(avg_new, new_model)
            avg_old_str = _format_vocab_distribution(avg_old, old_model)
            traj_msg = (
              f"epoch={epoch_num}\tstep={_step}\tt={random_t}\tsample={b_idx}\tlen={valid_len}\t"
        ...
        if args.mask_base_ce_coeff > 0 and random_t >= mask_base_start_t:
          if args.gradient_type == "motif_soft":
            step_mask_ce = _masked_motif_vocab_ce_loss(
              log_p_x0=log_p_x0,
              ...
              vocab_topk=args.mask_base_vocab_topk,
              topk_source=args.mask_base_vocab_topk_source,
            )
          else:
        ...
        if args.ood_enable and random_t >= ood_start_t:
          # a = last_x[:, :, :-1].argmax(dim=-1)
          # nll = -log_p_x0_old.gather(-1, a.unsqueeze(-1)).squeeze(-1)  # [B, L]
    ...
    ood_frac_tensor = torch.tensor(ood_frac_list) if ood_frac_list else None
    ood_score_mean_tensor = torch.tensor(ood_score_mean_list) if ood_score_mean_list else None
    drift_final_mask_new_t = torch.tensor(drift_final_mask_new) if drift_final_mask_new else None
    drift_final_mask_old_t = torch.tensor(drift_final_mask_old) if drift_final_mask_old else None
    drift_mask_curve_l1_t = torch.tensor(drift_mask_curve_l1) if drift_mask_curve_l1 else None
    drift_seq_hamming_t = torch.tensor(drift_seq_hamming) if drift_seq_hamming else None
    drift_seq_basecomp_l1_t = torch.tensor(drift_seq_basecomp_l1) if drift_seq_basecomp_l1 else None
    drift_seq_kmer_l1_t = torch.tensor(drift_seq_kmer_l1) if drift_seq_kmer_l1 else None
    ...
    print("Epoch %d" % epoch_num,
          ...
          "OOD frac %f" % (ood_frac_tensor.float().mean().item() if ood_frac_tensor is not None else 0.0),
          "OOD score mean %f" % (ood_score_mean_tensor.float().mean().item() if ood_score_mean_tensor is not None else 0.0),
          "Mask final(new) %f" % (drift_final_mask_new_t.float().mean().item() if drift_final_mask_new_t is not None else 0.0),
          "Mask final(old) %f" % (drift_final_mask_old_t.float().mean().item() if drift_final_mask_old_t is not None else 0.0),
          "Mask curve L1 %f" % (drift_mask_curve_l1_t.float().mean().item() if drift_mask_curve_l1_t is not None else 0.0),
          "Seq Hamming %f" % (drift_seq_hamming_t.float().mean().item() if drift_seq_hamming_t is not None else 0.0),
          "Seq basecomp L1 %f" % (drift_seq_basecomp_l1_t.float().mean().item() if drift_seq_basecomp_l1_t is not None else 0.0),
          "Seq kmer L1 %f" % (drift_seq_kmer_l1_t.float().mean().item() if drift_seq_kmer_l1_t is not None else 0.0))
    if args.name != 'debug':
      wandb.log({
        "epoch": epoch_num,
        ...
        "ood_frac": (ood_frac_tensor.float().mean().item() if ood_frac_tensor is not None else 0.0),
        "ood_score_mean": (ood_score_mean_tensor.float().mean().item() if ood_score_mean_tensor is not None else 0.0),
        "drift_final_mask_new": (drift_final_mask_new_t.float().mean().item() if drift_final_mask_new_t is not None else 0.0),
        "drift_final_mask_old": (drift_final_mask_old_t.float().mean().item() if drift_final_mask_old_t is not None else 0.0),
        "drift_mask_curve_l1": (drift_mask_curve_l1_t.float().mean().item() if drift_mask_curve_l1_t is not None else 0.0),
        "drift_seq_hamming": (drift_seq_hamming_t.float().mean().item() if drift_seq_hamming_t is not None else 0.0),
        "drift_seq_basecomp_l1": (drift_seq_basecomp_l1_t.float().mean().item() if drift_seq_basecomp_l1_t is not None else 0.0),
        "drift_seq_kmer_l1": (drift_seq_kmer_l1_t.float().mean().item() if drift_seq_kmer_l1_t is not None else 0.0),
      })
    with open(log_path, 'a') as f:
      f.write(
        ...
        f"OOD frac {(ood_frac_tensor.float().mean().item() if ood_frac_tensor is not None else 0.0)} "
        f"OOD score mean {(ood_score_mean_tensor.float().mean().item() if ood_score_mean_tensor is not None else 0.0)} "
        f"drift_final_mask_new {(drift_final_mask_new_t.float().mean().item() if drift_final_mask_new_t is not None else 0.0)} "
        f"drift_final_mask_old {(drift_final_mask_old_t.float().mean().item() if drift_final_mask_old_t is not None else 0.0)} "
        f"drift_mask_curve_l1 {(drift_mask_curve_l1_t.float().mean().item() if drift_mask_curve_l1_t is not None else 0.0)} "
        f"drift_seq_hamming {(drift_seq_hamming_t.float().mean().item() if drift_seq_hamming_t is not None else 0.0)} "
        f"drift_seq_basecomp_l1 {(drift_seq_basecomp_l1_t.float().mean().item() if drift_seq_basecomp_l1_t is not None else 0.0)} "
        f"drift_seq_kmer_l1 {(drift_seq_kmer_l1_t.float().mean().item() if drift_seq_kmer_l1_t is not None else 0.0)}\n")
...
argparser.add_argument('--gradient_type', type=str, choices=['base_soft', 'motif_soft'], default='motif_soft',
...
argparser.add_argument('--alpha', type=float, default=0.0015)
...
argparser.add_argument(
  ...
  type=str,
  default='/home/xli263/xli/utr_design/DRAKES/drakes_rna/data_and_model/token_length_distribution_50_utr_motif.txt',
...
argparser.add_argument('--log_denoise_trajectory', type=str2bool, default=True,
...
argparser.add_argument('--trajectory_motif_topk', type=int, default=10,
                       help='Top-k motif vocab probabilities to print per token position in trajectory logs.')
...
argparser.add_argument('--mask_base_ce_coeff', type=float, default=0.5,
...
argparser.add_argument('--mask_base_truncate_steps', type=int, default=0,
                       help='If >0, compute/average masked-base CE/KL over only the last K denoising steps. 0 means all steps.')
argparser.add_argument('--mask_base_vocab_topk', type=int, default=8,
                       help='For motif_soft CE/KL: if >0, keep only top-k token probs per position before divergence. 0 means full vocab.')
argparser.add_argument('--mask_base_vocab_topk_source', type=str, choices=['old', 'new', 'union'], default='new',
                       help='Reference distribution used to select top-k tokens for motif_soft CE/KL.')
argparser.add_argument('--enable_drift_monitor', type=str2bool, default=True,
...
if args.gradient_type != 'motif_soft':
  raise ValueError(
    "finetune_reward_te_ood_motif.py is motif-only. "
    "Please set --gradient_type motif_soft."
  )

CKPT_PATH = os.path.join(args.base_path, '/home/xli263/xli/utr_design/DRAKES/drakes_rna/mdlm/pretrained_utr_ckpt/pretrained_motif_50_utr_all.ckpt')
# log_base_dir = os.path.join(args.base_path, 'mdlm/reward_mrl_optimized_motif_50_utr_target_base_len')
log_base_dir = os.path.join(
    args.base_path,
    f"/home/xli263/xli/utr_design/DRAKES/drakes_rna/mdlm/reward_motif_50utr_ce_lr{args.learning_rate}_alpha{args.alpha}_t{args.truncate_steps}/"
    ...
    f"_topk{args.mask_base_vocab_topk}_{args.mask_base_vocab_topk_source}"
)
...
if args.name == 'debug':
  ...
  os.makedirs(save_path, exist_ok=True)
  wandb.init(project='reward_mrl_motif_ce', name=run_name, config=args, dir=save_path)
  log_path = os.path.join(save_path, f'log_seed{args.seed}_{curr_time}.txt')
  trajectory_log_path = os.path.join(save_path, f'trajectory_seed{args.seed}_{curr_time}.txt')
...
if args.reward_type == 'utrlm':
  ...
  reward_model = UTROracleWrapper(
      oracle_utr.get_utr_oracle(map_location=oracle_device,oracle_ckpt_path="/home/xli263/xli/utr_design/DRAKES/drakes_rna/data_and_model/utr_oracle/reward_utr_rl_fold3.ckpt"))
  # reward_model = UTROracleWrapper(
...
fine_tune(new_model, reward_model, reward_model_eval, old_model, args, eps_ood=eps_ood)
