...
# PyTorch 2.6+ defaults weights_only=True, which breaks several legacy
# checkpoints used by this project.
...
def _sample_to_base_probs(sample):
...
def _apply_reward_padding_side(base_probs, target_length, padding_side):
  """
  Convert the diffusion model's right-padded valid prefix into the reward
  model's requested alignment. For MTTrans, public runs use left padding:
  valid bases are shifted to the right side of the model-length window before
  the MTTrans wrapper pads/crops to its fixed input length.
  """
  if padding_side == "right" or target_length is None:
    return base_probs
  if padding_side != "left":
    raise ValueError(f"Unsupported reward padding side: {padding_side}")

  batch_size, seq_len, channels = base_probs.shape
  valid_lens = _get_valid_lens(target_length, batch_size=batch_size, max_len=seq_len)
  aligned = base_probs.new_zeros(batch_size, seq_len, channels)
  for batch_i, valid_len in enumerate(valid_lens):
    if valid_len <= 0:
      continue
    aligned[batch_i, seq_len - valid_len:] = base_probs[batch_i, :valid_len]
  return aligned
...
def _load_csv_sequences(csv_path: str, seq_col: str = "utr"):
  with open(csv_path, newline="") as handle:
    ...
    if reader.fieldnames is None or seq_col not in reader.fieldnames:
      raise ValueError(
        f"Sequence column '{seq_col}' not found in {csv_path}. "
        f"Columns: {reader.fieldnames}"
      )
    return [row[seq_col] for row in reader if row.get(seq_col)]
...
def configure_output(args):
  ...
  if args.log_base_dir:
    ...
    log_base_dir = os.path.join(
      args.output_dir,
      f"reward_te_mttrans_{args.learning_rate}_{diver_tag}_t{args.truncate_steps}",
      f"seed{args.seed}",
    )
...
def load_diffusion_models(args):
  ...
  initialize(config_path="configs_gosai", job_name="PILOT_UTR_TE_MTTrans_finetune")
...
def load_reward_models(args, device):
  if args.reward_type != "mttrans":
    raise ValueError("finetune_reward_te_new.py supports only --reward_type mttrans")
...
def _compute_kl_losses(new_model, old_model, last_x_list, condt_list, move_chance_t_list,
                       copy_flag_list, length_mask, args):
  total_kl = []
  total_kl_for = []
  for random_t in range(args.total_num_steps):
    ...
    total_kl_for.append((kl_div_for * last_x[:, :, :-1]).sum((1, 2)))

  return (
    torch.stack(total_kl, dim=1).sum(dim=1).mean(),
    torch.stack(total_kl_for, dim=1).sum(dim=1).mean(),
  )


def _entropy_from_last_probs(p_vocab, log_p_x0_last):
...
def finetune(new_model, old_model, reward_model, reward_model_eval, args, save_path, log_path):
  del reward_model_eval
  with open(log_path, "w") as handle:
    handle.write(repr(args) + "\n")

  if args.gradient_type != "base_soft":
    raise ValueError("The trimmed TE MTTrans script supports only --gradient_type base_soft")
  if args.mask_base_ce_coeff != 0:
    raise ValueError("Masked-base regularization was removed from the trimmed release script.")
  ...
  for epoch_num in range(args.num_epochs):
    ...
    for accum_step in range(args.num_accum_steps):
      ...
      with torch.no_grad():
        base_probs_metric = _sample_to_base_probs(sample.detach())
      ...
      base_probs = _sample_to_base_probs(sample)
      base_probs_reward = _apply_reward_padding_side(
        base_probs, target_length, args.reward_padding_side).transpose(1, 2)
      preds = reward_model(base_probs_reward, soft_input=True)
      reward = preds[..., 0]

      seq_len = kl_x_list[0].shape[1]
      ...
      kl_loss, kl_loss_for = _compute_kl_losses(
        new_model,
        old_model,
        kl_x_list,
        condt_list,
      ...
      entropy = _entropy_from_last_probs(p_vocab, log_p_x0_last)
    ...
    eligible = (
      eval_kmer_corr is None
      or eval_kmer_corr >= args.save_best_min_eval_kmer_corr
    )
    metric_value = metrics[f"mean_{args.save_best_metric}"]
    if eligible and metric_value > best_metric_value:
      best_metric_value = metric_value
      torch.save(new_model.state_dict(), best_checkpoint_path)
      print(
        f"New best ({args.save_best_metric}={best_metric_value:.6f}, "
        f"eval_kmer_corr={eval_kmer_corr}) saved to {best_checkpoint_path}"
      )
...
def parse_args():
  parser = argparse.ArgumentParser(
    description="PILOT-UTR TE reward-guided diffusion finetuning with MTTrans."
  )
  parser.add_argument("--config_name", default="config_gosai_pretrain")
  parser.add_argument("--pretrained_ckpt_path", required=True)
  parser.add_argument("--mttrans_checkpoint", required=True)
  parser.add_argument("--mttrans_input_length", type=int, default=105)
  parser.add_argument("--reward_type", choices=["mttrans"], default="mttrans")
  parser.add_argument("--reward_padding_side", choices=["right", "left"], default="left")
  ...
  parser.add_argument("--name", default="pilot_utr_te_mttrans")
  ...
  parser.add_argument("--save_best_metric", choices=["reward", "reward_eval"], default="reward_eval")
  parser.add_argument("--save_best_min_eval_kmer_corr", type=float, default=0.70)
  parser.add_argument("--gradient_type", choices=["base_soft"], default="base_soft")
  parser.add_argument("--forward_kl_on_old_xt", type=str2bool, default=False)
  parser.add_argument("--mask_base_ce_coeff", type=float, default=0.0)
  parser.add_argument("--mask_base_agg_method", choices=["global", "position"], default="global")
  parser.add_argument("--mask_base_divergence", choices=["ce", "kl"], default="ce")
  parser.add_argument("--mask_base_truncate_steps", type=int, default=50)
