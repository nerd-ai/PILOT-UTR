...
class Diffusion(L.LightningModule):
  def __init__(
    ...
    self.pad_token_id = getattr(self.config.data, 'pad_token_id', self.mask_index)
  ...
  def _compute_loss(self, batch, prefix):
    ...
    labels = batch['clss'] if 'clss' in batch else batch['labels']
    if labels.ndim == 1:
      labels = labels[:, None]
    binary_clss = (labels[:,0] > self.config.model.cls_free_threshold).long()
  ...
  def _prepare_target_length(self, target_length, batch_size):
    if target_length is None:
      return None
    if isinstance(target_length, int):
      target_length = torch.full(
        (batch_size,), target_length, device=self.device, dtype=torch.long)
      ...
      target_length = target_length.to(device=self.device, dtype=torch.long)
    else:
      raise TypeError(f"Unsupported target_length type: {type(target_length)}")
    return target_length.clamp(min=0, max=self.config.model.length)

  def _enforce_target_length_ids(self, x_ids, target_length):
    if target_length is None or self.pad_token_id is None:
      return x_ids
    seq_indices = torch.arange(x_ids.shape[1], device=x_ids.device).unsqueeze(0)
    ...
    if self.eos_token_id is None:
      ...
      pad_mask = seq_indices > target_length_exp
      x_ids = torch.where(eos_mask, torch.full_like(x_ids, self.eos_token_id), x_ids)
    return torch.where(pad_mask, torch.full_like(x_ids, self.pad_token_id), x_ids)
  ...
  @torch.no_grad()
  def _sample(self, num_steps=None, eps=1e-5, eval_sp_size=None, cls=1, w=None, target_length=None):
    ...
    target_length = self._prepare_target_length(target_length, batch_size_per_gpu)
    x = self._enforce_target_length_ids(x, target_length)
