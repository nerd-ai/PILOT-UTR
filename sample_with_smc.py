...
def _get_utrlm_te_vanilla_checkpoints(root_dir: str) -> List[str]:
    root = Path(root_dir)
    expected = {
...
def parse_target_length(args, eos_id: Optional[int]):
    specified = [
        args.target_nt_length is not None,
        args.target_token_length is not None,
        args.target_length_distribution is not None,
    ]
    if sum(bool(x) for x in specified) > 1:
        raise ValueError(
            "Specify only one of --target-nt-length, --target-token-length, or --target-length-distribution."
        )
...
def load_target_length_distribution(distribution_path: Path, eos_id: Optional[int]) -> Tuple[List[int], List[float]]:
    raw = distribution_path.read_text().strip()
    prefix = "probs :"
    if not raw.startswith(prefix):
        raise ValueError(
            f"Expected distribution file starting with '{prefix}', got: {distribution_path}"
        )
    dist = ast.literal_eval(raw[len(prefix):].strip())
    if not isinstance(dist, dict) or not dist:
        raise ValueError(f"Expected a non-empty dict in {distribution_path}")

    lengths = []
    probs = []
    for key, value in sorted(dist.items(), key=lambda kv: int(kv[0])):
        token_length = int(key)
        prob = float(value)
        if prob <= 0:
            continue
        target_length = max(0, token_length - 1) if eos_id is not None else token_length
        lengths.append(target_length)
        probs.append(prob)

    if not lengths:
        raise ValueError(f"No positive-probability lengths found in {distribution_path}")
...
def load_target_token_lengths(
    ...
    for seq in df[seq_column].astype(str):
        ...
        # With no EOS, target_length is the number of real tokens before PAD.
        # With EOS, it is the position where EOS is forced.
...
def sample_target_lengths(lengths: Sequence[int], probs: Sequence[float], size: int) -> List[int]:
    return [int(x) for x in np.random.choice(lengths, size=size, p=probs)]
...
def main():
    parser = argparse.ArgumentParser("Generate sequences with SMC guidance over a pretrained diffusion model.")
    ...
    parser.add_argument("--alpha", type=float, default=1.0, help="SMC resampling temperature.")
    ...
    parser.add_argument("--target-length-distribution", type=Path, default=None,
                        help="Path to a token-length distribution file of the form 'probs : {length: prob, ...}'. If EOS is absent in the tokenizer, lengths are used directly.")
    parser.add_argument("--sample-target-lengths", action=argparse.BooleanOptionalAction, default=True,
                        help="Sample target token lengths from --length-source-csv when no fixed/distribution target length is set.")
    ...
    parser.add_argument("--reward-type", type=str, choices=["utrlm", "utrlm_te", "te", "rnafm", "framepool", "mttrans"], default="utrlm")
    parser.add_argument("--oracle-ckpt-path", type=str,
                        default="/home/xli263/xli/utr_design/DRAKES/drakes_rna/experiment/single_run_vanilla_standard/vanilla_best.ckpt")
    parser.add_argument("--utrlm-te-vanilla-root", type=str,
    ...
    target_length_distribution = None
    if args.target_length_distribution is not None:
        target_length_distribution = load_target_length_distribution(
            args.target_length_distribution,
            eos_id=eos_id,
        )
    target_length_values = None
    if target_length is None and target_length_distribution is None and args.sample_target_lengths:
    ...
    while remaining > 0:
        ...
        if target_length is not None:
            batch_target_lengths = [int(target_length)] * batch_size
        elif target_length_distribution is not None:
            batch_target_lengths = sample_target_lengths(
                target_length_distribution[0],
                target_length_distribution[1],
                size=batch_size,
            )
        elif target_length_values is not None:
            idx = torch.randint(0, len(target_length_values), (batch_size,))
            batch_target_lengths = target_length_values[idx].tolist()
        else:
            batch_target_lengths = [None] * batch_size

        samples_per_length = {}
        for sampled_length in sorted(set(batch_target_lengths)):
            count = batch_target_lengths.count(sampled_length)
            samples_per_length[sampled_length] = model.controlled_sample_SMC(
                reward_model=reward_model,
                alpha=float(args.alpha),
                num_steps=args.num_steps,
                eval_sp_size=count,
                target_length=sampled_length,
            )

        ordered_samples = []
        offsets = {length: 0 for length in samples_per_length}
        for sampled_length in batch_target_lengths:
            idx = offsets[sampled_length]
            ordered_samples.append(samples_per_length[sampled_length][idx])
            offsets[sampled_length] += 1
        samples = torch.stack(ordered_samples, dim=0)
        ...
        for seq, reward, token_ids, sampled_length in zip(seqs, rewards, trimmed_ids, batch_target_lengths):
            rows.append({
                "seq": seq,
                "reward": float(reward),
                "target_length": sampled_length,
                "token_length": len(token_ids) + (1 if eos_id is not None and sampled_length is not None else 0),
                "nt_length": len(seq),
                "token_ids": " ".join(str(token_id) for token_id in token_ids),
            })
        remaining -= batch_size
        unique_lengths = sorted(set(batch_target_lengths))
        print(
            f"Generated {len(rows)}/{args.num_samples} sequences"
            f" (target_lengths={unique_lengths[:8]}{'...' if len(unique_lengths) > 8 else ''})"
        )
