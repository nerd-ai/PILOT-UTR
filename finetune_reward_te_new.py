...
def _apply_reward_padding_side(sample_probs, target_length, padding_side):
  """
  Re-align [B, L, C] sampled probabilities before reward scoring.

  The diffusion model can keep valid tokens left-aligned with right padding.
  For reward models that expect the valid suffix near the right edge, this
  helper moves the valid prefix to the right so the visible [0,0,0,0] padding
  sits on the left. Gradient still flows back to the original valid positions.
  """
  if padding_side == "right" or target_length is None:
    return sample_probs

  if padding_side != "left":
    raise ValueError(f"Unsupported reward_padding_side: {padding_side}")

  B, L, C = sample_probs.shape
  out = torch.zeros_like(sample_probs)
  if isinstance(target_length, torch.Tensor):
    valid_lens = target_length.to(sample_probs.device, dtype=torch.long).view(-1)
  elif isinstance(target_length, (list, tuple)):
    valid_lens = torch.as_tensor(target_length, device=sample_probs.device, dtype=torch.long).view(-1)
  else:
    valid_lens = torch.full((B,), int(target_length), device=sample_probs.device, dtype=torch.long)
  ...
  for bi in range(B):
    ...
    if cur_len <= 0:
      continue
    out[bi, L - cur_len:, :] = sample_probs[bi, :cur_len, :]
  return out
...
def _pearson_corr(x_vals, y_vals) -> float:
  if len(x_vals) != len(y_vals) or len(x_vals) == 0:
    return 0.0
  mx = sum(x_vals) / float(len(x_vals))
  my = sum(y_vals) / float(len(y_vals))
  num = 0.0
  den_x = 0.0
  den_y = 0.0
...
def _get_utrlm_te_vanilla_checkpoints(root_dir: str):
  root = Path(root_dir)
  expected = {
    "HEK": root / "HEK" / "seed_42" / "vanilla_te_best.ckpt",
    "Muscle": root / "Muscle" / "seed_42" / "vanilla_te_best.ckpt",
    "pc3": root / "pc3" / "seed_42" / "vanilla_te_best.ckpt",
  }
  missing = [str(path) for path in expected.values() if not path.exists()]
  if missing:
    raise FileNotFoundError(
      "Missing required UTR-LM TE checkpoints: " + ", ".join(missing)
    )
  return [str(expected["HEK"]), str(expected["Muscle"]), str(expected["pc3"])]
...
def fine_tune(new_model, new_model_y, new_model_y_eval, old_model, args, eps_ood=None, eps=1e-5, sft_loader=None):
  ...
  initial_lr = args.lr_warmup_start if args.lr_warmup_epochs > 0 else args.learning_rate
  optim = torch.optim.Adam(new_model.parameters(), lr=initial_lr)
  scheduler = None
  if args.lr_warmup_epochs > 0:
    warmup_epochs = max(1, int(args.lr_warmup_epochs))
    warmup_start = float(args.lr_warmup_start)
    warmup_target = float(args.learning_rate)

    def _lr_lambda(epoch):
      if epoch < warmup_epochs:
        progress = float(epoch + 1) / float(warmup_epochs)
        current = warmup_start + (warmup_target - warmup_start) * progress
        return current / warmup_start if warmup_start > 0 else 1.0
      return warmup_target / warmup_start if warmup_start > 0 else 1.0

    scheduler = torch.optim.lr_scheduler.LambdaLR(optim, lr_lambda=_lr_lambda)
  elif args.lr_cosine_decay:
    T = 100
    lr_min = args.lr_min
  ...
  for epoch_num in range(args.num_epochs):
    ...
    for _step in range(args.num_accum_steps):
      ...
      if args.gradient_type == "motif_soft" and new_model.vocab_size > 7:
        ...
        base_probs = sample
      base_probs = _apply_reward_padding_side(base_probs, target_length, args.reward_padding_side)
      base_probs = base_probs.transpose(1, 2)
      # if _step == 0:
      #   print(
      #     "[tokens]",
      #     sample[0, : min(sample.shape[1], 16)].detach().cpu().tolist()
      #   )
      #   print(
      #     "[base_probs]",
      #     base_probs[0, :, : min(base_probs.shape[-1], 8)].detach().cpu().transpose(0, 1).tolist()
      #   )


      use_soft = args.reward_type in ('utrlm', 'utrlm_ensemble', 'utrlm_te', 'rnafm', 'framepool', 'mttrans')
    ...
    # Track and save best checkpoint: require sufficient eval k-mer correlation,
    # then choose the highest selected reward metric among eligible epochs.
    mean_reward = rewards.float().mean().item()
    mean_reward_eval = rewards_eval.float().mean().item()
    metric_by_name = {
      "reward": mean_reward,
      "reward_eval": mean_reward_eval,
    }
    metric_value = metric_by_name[args.save_best_metric]
    kmer_pass = (
      eval_kmer_corr is not None
      and eval_kmer_corr > args.save_best_min_eval_kmer_corr
    )
    if kmer_pass and metric_value > best_metric:
      best_metric = metric_value
      torch.save(new_model.state_dict(), best_checkpoint_path)
      print(
        f"New best ({args.save_best_metric}={best_metric:.4f}, "
        f"eval_{args.eval_kmer_k}mer_corr={eval_kmer_corr:.4f}) "
...
argparser.add_argument('--lr_warmup_epochs', type=int, default=0,
                       help='Number of warmup epochs using a smaller LR before switching to --learning_rate.')
argparser.add_argument('--lr_warmup_start', type=float, default=1e-5,
                       help='Starting LR used during warmup when --lr_warmup_epochs > 0.')
...
argparser.add_argument('--reward_type', type=str, choices=['utrlm', 'utrlm_ensemble', 'utrlm_te', 'rnafm', 'framepool', 'mttrans'], default='utrlm')
...
argparser.add_argument('--save_every_n_epochs', type=int, default=1)
...
argparser.add_argument('--save_best_metric', type=str, choices=['reward', 'reward_eval'], default='reward_eval',
                       help='Reward metric used to rank eligible best checkpoints.')
argparser.add_argument('--save_best_min_eval_kmer_corr', type=float, default=0.70,
                       help='Minimum eval k-mer correlation required before an epoch can replace best_model.ckpt.')
...
argparser.add_argument('--pretrained_ckpt_path', type=str, default=None,
                       help='Optional path to the pretrained diffusion checkpoint. Defaults to pretrained_4_base_all_utr.ckpt when omitted.')
argparser.add_argument('--utrlm_te_vanilla_root', type=str,
                       default="/home/xli263/xli/utr_design/DRAKES/drakes_rna/experiment/single_run_vanilla_te",
                       help='Root directory containing HEK/Muscle/pc3 vanilla TE checkpoints.')
...
argparser.add_argument('--oracle_ckpt_paths', type=str, nargs='+', default=None,
                       help='Multiple UTR oracle checkpoint paths for ensemble reward guidance.')
argparser.add_argument('--utrlm_eval_checkpoint_paths', type=str, nargs='+', default=None,
                       help='Optional explicit UTR-LM eval checkpoint paths to average for reward_type=utrlm_ensemble.')
...
argparser.add_argument('--mttrans_checkpoint', type=str,
                       default='/home/xli263/xli/utr_design/UTRGAN/src/mrl_te_optimization/script/checkpoint/RL_hard_share_MTL/3R/schedule_MTL-model_best_cv1.pth',
                       help='Path to the serialized MTTrans/UTRGAN TE reward checkpoint.')
argparser.add_argument('--mttrans_input_length', type=int, default=105,
                       help='Fixed input length for MTTrans reward preprocessing (left-pad / last-N trim).')
argparser.add_argument('--reward_padding_side', type=str, choices=['right', 'left'], default='right',
                       help='Re-align sampled [A,C,G,T] probabilities before reward scoring. '
                            '"left" moves zero padding to the left of the valid tokens.')
...
CKPT_PATH = args.pretrained_ckpt_path or '/home/xli263/xli/utr_design/DRAKES/drakes_rna/mdlm/pretrained_utr_ckpt/pretrained_4_base_all_utr.ckpt'
...
if args.log_base_dir is not None:
  ...
  log_base_dir = os.path.join(
      "/home/xli263/xli/utr_design/DRAKES/drakes_rna/mdlm_mrl_25_100_ensemble",
      f"reward_4_base_utr_45_100_{args.reward_type}_{args.learning_rate}_{diver_tag}_t{args.truncate_steps}",
      f"seed{args.seed}"
  )
...
elif args.reward_type == 'utrlm_ensemble':
  print("Using UTR_Oracle ensemble")
  oracle_device = str(getattr(new_model, "device", "cuda"))
  if not args.oracle_ckpt_paths or len(args.oracle_ckpt_paths) < 2:
    raise ValueError("reward_type=utrlm_ensemble requires at least two --oracle_ckpt_paths.")
  reward_model = UTROracleWrapper(
      oracle_utr.get_utr_oracle(
          map_location=oracle_device,
          oracle_ckpt_paths=args.oracle_ckpt_paths))
  default_eval_ckpt = "/home/xli263/xli/utr_design/UTR-LM/Model/Downstream/MRL/MJ3_seed1337_ESM2SISS_FS4.1.ep93.1e-2.dr5_unmod_1_utr_10folds_rl_LabelScalerFalse_LabelLog2False_AvgEmbFalse_BosEmbTrue_CNNlayer0_epoch300_nodes40_dropout30.5_finetuneTrue_huberlossTrue_lr0.01_fold0_epoch299.pt"
  eval_ckpt_paths = args.utrlm_eval_checkpoint_paths
  if eval_ckpt_paths is None:
    eval_ckpt_paths = [default_eval_ckpt]
  reward_model_eval = oracle_new.get_utrlm_oracle(
      checkpoint_root=args.utrlm_checkpoint_root,
      checkpoint_paths=eval_ckpt_paths,
      device=oracle_device,
      seq_trim_len=args.utrlm_seq_trim_len)
elif args.reward_type == 'utrlm_te':
  oracle_device = str(getattr(new_model, "device", "cuda"))
  te_ckpt_paths = _get_utrlm_te_vanilla_checkpoints(args.utrlm_te_vanilla_root)
  reward_model = UTROracleWrapper(
      oracle_utr.get_utr_oracle(
          map_location=oracle_device,
