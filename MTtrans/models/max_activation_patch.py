...
from MTtrans import utils as utils
import copy
import torch
import train_val
import reader
...
#         print(self.X_ls.shape)
    ...
    def retrieve_featmap_at_changepoint(self, featmap, rl_chain, threshold=1, direction='less',
                                        detect_region=None, source_indices=None, return_metadata=False):
        """
        ...
        if source_indices is None:
            source_indices = np.arange(featmap.shape[0])

        change_point_act = []
        metadata = []
        for i, trend in enumerate(rl_chain):
            region = detect_region[i]
            segment = trend[region]
            if segment.size<2:
                continue   
            try:
                region_start = 0 if region.start is None else region.start
                bkpt= self.detect_changepoint(segment) + region_start
            ...
            if condition(trend[bkpt+1] , trend[bkpt]): 
                ...
                delta = float(trend[bkpt+1] - trend[bkpt])
                metadata.append((int(source_indices[i]), int(bkpt+1), delta))

        chagnepoint_map = np.asarray(change_point_act)
        if return_metadata:
            return chagnepoint_map, metadata
